...
public class UmsStudentSeviceImpl implements UmsStudentService {
    ...
    @Value("$(ums.default.student.password)")
    private String DEFAULT_STUDENT_PASSWORD;
    ...
    @Override
    public int updateStudentMassively(List<UmsStudent> umsStudents) {
        int count=0;
        for(UmsStudent umsStudent:umsStudents){
            updateStudent(umsStudent);
            count++;
        }
        return count;
    }
    ...
    @Override
    public UmsStudent updateStudent(UmsStudent umsStudent) {
        if(umsStudent.getId()==null){
            UmsStudentExample example = new UmsStudentExample();
            example.createCriteria().andUsernameEqualTo(umsStudent.getUsername());
            List<UmsStudent>students = umsStudentMapper.selectByExample(example);
            if(students!=null&&students.size()>0){
                return null;
            }
            if (umsStudent.getPassword()==null)
                umsStudent.setPassword(DEFAULT_STUDENT_PASSWORD);
            //将密码进行加密操作
        ...
        umsStudentMapper.updateByPrimaryKeySelective(umsStudent);
        return umsStudent;
    }

    @Override
    public void deleteStudent(long id) {
        umsStudentMapper.deleteByPrimaryKey(id);
    }
